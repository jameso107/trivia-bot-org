...
export const ROLES: Record<RoleKey, RoleDef> = {
  ...
  "venue-search": {
    ...
    tools: ["query_org", "insert_lead", "update_lead", "create_task", "update_task"],
    webSearch: true,
    goal: `Research-only lead building per your role card and the ICP: use web search to find Metro Detroit venues that fit, verify each with REAL source URLs (never fabricate evidence — policies §3), extract decision-maker contact only where public, score per the rubric, and insert_lead each one. Claim your open task rows and update them with counts. Target this run: 3-5 QUALITY leads (Phase-A budget beats volume). NO outreach of any kind.`,
  },
  ...
  "ads-recruit": {
    ...
    tools: ["query_org", "query_product", "upsert_sponsor", "create_task"],
    webSearch: true,
